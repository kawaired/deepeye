Shader "Unlit/face"
{
    Properties
    {
        ...
        _ShadowParam("shadowparam",range(0,1))=0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float3 worldfront:texcoord4;
                float3 worldleft:texcoord5;
            ...
            v2f vert (appdata v)
            {
                ...
                o.worldfront=UnityObjectToWorldDir(float3(0,1,0)); 
                o.worldleft=UnityObjectToWorldDir(float3(0,0,-1));
            ...
            {
                //return i.worldleft.xyzz;
                //return i.worldfront.xyzz;
               //return float4(i.sidenormal.xyz,1);
                ...
                float3 lightdir=normalize(_WorldSpaceLightPos0);
                 float3 cuslight=normalize(float3(lightdir.x,0,lightdir.z));
                 float facefac=dot(cuslight,i.worldleft);
                 float frontfactor=dot(cuslight,i.worldfront);
                float4 masktex=(facefac>0)*tex2D(_MaskTex,float2(1-i.uv.x,i.uv.y))+(facefac<=0)*tex2D(_MaskTex,i.uv);
                mapmsg.x=((frontfactor+masktex.w)-_ShadowParam);
                mapmsg.x=clamp(mapmsg.x,0.03,0.97);
                float4 ramptex=tex2D(_ShadowRampTex,mapmsg.xy);
                float4 rampcolor=ramptex*pbr;
                rampcolor.xyz=rampcolor.xyz*1.1;
                // return masktex.w;
                return rampcolor;
